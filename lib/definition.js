...
const { ALIAS_PATTERN, TOKEN_REF_PATTERN } = require("./constants");
...
const transform = (def, options) => {
  ...
  const go = def =>
    Either.fromNullable(def)
      ...
      .chain(
        def => Either.try(resolveTokenReferences)(def)
      )
      .map(addPropName);
  /**
  ...
  /**
   * Resolve token references, values named as "{$token_name}", with the value of that token.
   */
  const resolveTokenReferences = def => {
    return def.update("props", props => {
      return props.map((value, _key) => {
        return value.update("value", v => {
          return recursiveMap(v, v => {
            if (typeof v !== "string") return v;
            return allMatches(v, TOKEN_REF_PATTERN).reduce((v, [prop, key]) => {
              if (!props.has(key))
                throw new Error(`Prop reference "${key}" not found in props`);
              return v.replace(prop, props.getIn([key, "value"]));
            }, v);
          });
        });
      });
